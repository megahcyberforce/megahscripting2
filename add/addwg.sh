...
SERVER_PUB_IP=$(wget -qO- ifconfig.me/ip);
	echo "Type An Username For Wireguard Client."
	echo "Please Make Sure There's No Special Characters."
	...
	CLIENT_ADDRESS="10.1.0.$((LASTIP+1))"
	fi
...
echo -e "${RED}===================================================${NC}"
echo -e "${GREEN}Link Download Wireguard Config ${NC} : http://$MYIP:81/$CLIENT_NAME.conf"
echo -e "${GREEN}Expired Date${NC} : $exp"
echo -e "${RED}===================================================${NC}"
echo -e "${GREEN}Script By PakyaVPN${NC}"
echo -e "${GREEN}t.me/anakjati567${NC}"
echo -e "${RED}===================================================${NC}"
read -p "Press Enter For Back To Wireguard Menu/ CTRL+C To Cancel : "
wg-menu